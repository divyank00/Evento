...
import com.example.collegeproject.base.BaseActivity.animationPlaybackSpeed
import com.example.collegeproject.base.home.filterFAB.filter.utils.*
...
class FiltersLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    private val sheetHeight: Float by lazy { screenWidth.toFloat()-200 }
    ...
    private val tabsHandler: ViewPagerTabsHandler by lazy {
        ViewPagerTabsHandler(viewPager, tabLayout, bottomBarCardView)
    }
    ...
    init {
        ...
        fab.setOnClickListener {
            if ((context as BaseActivity).isAdapterFiltered) {
    ...
    private fun openFilterSheet(isOpening: Boolean) {
        ...
        val scaleDownAnimator = getAdapterScaleDownAnimator(isOpening)
...
//            tabsRecyclerView.translationY = tabsHeight * (1 - progress)
    ...
    private fun onFilterApplied() {
        ...
        (context as BaseActivity).filter()
        ...
        // Close icon rotates to simulate loading while adapter items in BaseActivity are filtered
        ...
        closeIconLoadingAnimator.doOnStart {
            ...
            (context as BaseActivity).isAdapterFiltered = true
        }
        ...
        val scaleDownAnimator = getAdapterScaleDownAnimator(false)
        scaleDownAnimator.duration = pathAnimator.duration

        // AnimatorSet choreography of the animation: (1,2,3) -> 4 -> (5,6)
        // At the end of the animation, BaseActivity shows it's fab. removes this fragment and we
    ...
    private fun unFilterAdapterWithFabAnimation() {
        (context as BaseActivity).unFilter()
        ...
        val scaleDownAnimator = getAdapterScaleDownAnimator(true)
        ...
        closeIconLoadingAnimator.doOnStart { (context as BaseActivity).isAdapterFiltered = false }
        ...
        val scaleUpAnimator = getAdapterScaleDownAnimator(false)
     ...
     * of animation, we ask MainListAdapter from BaseActivity to start it's animations as well.
     */
    ...
    private fun getAdapterScaleDownAnimator(isScaledDown: Boolean): ValueAnimator =
            getScaleDownAnimator(isScaledDown)
}

fun getScaleDownAnimator(isScaledDown: Boolean): ValueAnimator {
    return getValueAnimator(isScaledDown,
