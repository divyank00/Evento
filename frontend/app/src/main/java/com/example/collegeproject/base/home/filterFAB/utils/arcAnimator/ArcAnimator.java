package com.example.collegeproject.base.home.filterFAB.utils.arcAnimator;
...
public class ArcAnimator extends ValueAnimator {
    ...
    private ArcAnimator(ArcMetric arcmetric, View target) {
        ...
        addUpdateListener(animation -> setDegree((Float) animation.getAnimatedValue()));
    }
...
//                mValue = degree;
            ...
            target.setX(x - (float)target.getWidth() / 2);
            target.setY(y - (float)target.getHeight() / 2);
        });
    ...
    private void setDegree(float degree) {
        ...
        clipView.setX(x - (float)clipView.getWidth() / 2);
        clipView.setY(y - (float)clipView.getHeight() / 2);
    }
