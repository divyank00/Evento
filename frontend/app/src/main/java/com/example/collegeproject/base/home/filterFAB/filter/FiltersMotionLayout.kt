...
import com.example.collegeproject.base.home.filterFAB.filter.views.MultiListenerMotionLayout
...
class FiltersMotionLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    private fun unFilterAdapterItems(): Unit = performAnimation {
        ...
        (context as BaseActivity).isAdapterFiltered = false
    ...
    /**
     * Convenience method to launch a coroutine in BaseActivity's lifecycleScope
     ...
     * [animationPlaybackSpeed] in [BaseActivity] is changed using the Nav Drawer).
     */
    private fun updateDurations() = definedTransitions.forEach {
        it.duration = ((durationsMap[it] ?: error("")) / animationPlaybackSpeed).toInt()
    }
    ...
    private fun getAdapterScaleDownAnimator(isScaledDown: Boolean): ValueAnimator =
            getScaleDownAnimator1(isScaledDown)
}

fun getScaleDownAnimator1(isScaledDown: Boolean): ValueAnimator {
